...
class User(db.Model):
    ...
    def __init__(self,username,password,email,password_hash,confirmed=False):
...
def signin():
    ...
    if form.validate_on_submit():
        ...
        if psw1 != psw2:
            ...
            return redirect(url_for('home',err=1))
        try:
            newuser = User(username=username,password=psw1,email=email,password_hash=md5(psw1),confirmed=False)
